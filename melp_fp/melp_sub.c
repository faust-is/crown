...
/* External variables (constant arrays) */
extern const float bpf_num[];
extern const float bpf_den[];

void bpvc_ana(float speech[], float fpitch[], float bpvc[], float pitch[], float **bpfdel, float **envdel,  float *envdel2, float *sigbuf)
{
...
static const float dc_num[DC_ORD+1] = {    
      0.92692416,
     -3.70563834,
      5.55742893,
     -3.70563834,
      0.92692416};
static const float dc_den[DC_ORD+1] = {
       1.00000000,
     -3.84610723,
      5.55209760,
     -3.56516069,
...
void dc_rmv(float sigin[], float sigout[], float dcdel[], float* tmp_buf)
{
    ...
    v_equ(tmp_buf, dcdel, DC_ORD);
    polflt(sigin, dc_den, &tmp_buf[DC_ORD], DC_ORD, FRAME);
    v_equ(dcdel, &tmp_buf[FRAME], DC_ORD);
    zerflt(&tmp_buf[DC_ORD], dc_num, sigout, DC_ORD, FRAME);
}
...

{
    ...
    if (gain[0] > GN_QUP)
      gain[0] = GN_QUP;
    if (fabs(gain[1] - prev) < GAIN_INT_DB && 
	fabs(gain[0] - 0.5*(gain[1]+prev)) < 3.0) {

	/* interpolate and set special code */
	gain[0] = 0.5*(gain[1]+prev);
	gain_index[0] = 0;
    }
    else {
...
const double inv_scaleover = (1.0/SCALEOVER);
...

{
    ...
    for (i = 1; i < SCALEOVER; i++) {
        speech[i-1] *= ((scale*i + *prev_scale*(SCALEOVER-i)) * inv_scaleover);
    }
