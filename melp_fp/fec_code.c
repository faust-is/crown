...
/* Prototypes */
int16_t binprod_int(const int16_t *x, const int16_t *y, int16_t n);
int16_t *vgetbits(int16_t *y, int16_t x, int16_t p, int16_t n);
int16_t vsetbits(int16_t x, int16_t p, int16_t n, const int16_t *y);
void sbc_enc(int16_t x[], int16_t n, int16_t k, const int16_t *pmat);
int16_t sbc_dec(int16_t x[], int16_t n, int16_t k, const int16_t *pmat, const int16_t *syntab);
int16_t sbc_syn(const int16_t x[], int16_t n, int16_t k, const int16_t *pmat);
...
const int16_t pitch_enc[PIT_QLEV+1] = 
{
0x0, /* UV_PIND */
...
const int16_t pitch_dec[1<<PIT_BITS] = 
{
UV_PIND, /* 0x0 */
UV_PIND, /* 0x1 */
UV_PIND, /* 0x2 */
...
*/
/* (7,4) Hamming code parity matrix */
/* Parity generator matrix. */
const int16_t pmat74[3][4] = {{1,1,0,1},{1,0,1,1},{0,1,1,1}};

/* Syndrome table. */
 /* (7,4) Hamming code syndrome->bep table */
const int16_t syntab74[8] = {BEP_CORR,6,5,2,4,1,0,3};
...
/* Parity generator matrix. */
/* (8,4) Hamming code parity matrix */
const int16_t pmat84[4][4] = {{1,1,0,1},{1,0,1,1},{0,1,1,1},{1,1,1,0}};

/* Syndrome->error position lookup table. */
/* (8,4) Hamming code syndrome->bep table */
const int16_t syntab84[16] =
{
    BEP_CORR,    /* 0x0 */
    7,           /* 0x1 */
    6,           /* 0x2 */
...
** bpvc index.
*/
	vgetbits(par->codewd84,par->msvq_index[0],6,4);
	sbc_enc(par->codewd84,8,4,&pmat84[0][0]);
	par->bpvc_index=vsetbits(par->bpvc_index,3,4,&par->codewd84[4]);
/*
** Code 3 LSB of first vq stage index using (7,4) Hamming code; parity bits
** in 3 MSB of fsvq index.
*/
	vgetbits(par->codewd74,par->msvq_index[0],2,3);
	par->codewd74[3] = 0;
	sbc_enc(par->codewd74,7,4,&pmat74[0][0]);
	par->fsvq_index[0]=vsetbits(par->fsvq_index[0],7,3,&par->codewd74[4]);
/*
** Code 4 MSB of second gain index using (7,4) Hamming code; parity bits in
** next 3 MSB of fsvq index.
*/
	vgetbits(par->codewd74,par->gain_index[1],4,4);
	sbc_enc(par->codewd74,7,4,&pmat74[0][0]);
	par->fsvq_index[0]=vsetbits(par->fsvq_index[0],4,3,&par->codewd74[4]);
/*
** Code LSB of second gain index, first gain index using (7,4) Hamming code;
** parity bits in 2 LSB of fsvq index, jitter index bit.
*/
	vgetbits(par->codewd74,par->gain_index[1],0,1);
	vgetbits(&par->codewd74[1],par->gain_index[0],2,3);
	sbc_enc(par->codewd74,7,4,&pmat74[0][0]);
	par->fsvq_index[0]=vsetbits(par->fsvq_index[0],1,2,&par->codewd74[4]);
	par->jit_index=vsetbits(par->jit_index,0,1,&par->codewd74[6]);
    }
...
*/
	vgetbits(par->codewd84,par->msvq_index[0],6,4);
	vgetbits(&par->codewd84[4],par->bpvc_index,3,4);
	berr_pos=sbc_dec(par->codewd84,8,4,&pmat84[0][0],syntab84);
	erase |= berr_pos == BEP_UNCORR;
	par->msvq_index[0]=vsetbits(par->msvq_index[0],6,4,par->codewd84);
	par->bpvc_index = 0;
...
*/
	    vgetbits(par->codewd74,par->msvq_index[0],2,3);
	    par->codewd74[3] = 0;
	    vgetbits(&par->codewd74[4],par->fsvq_index[0],7,3);
	    berr_pos=sbc_dec(par->codewd74,7,4,&pmat74[0][0],syntab74);
	    par->msvq_index[0]=vsetbits(par->msvq_index[0],2,3,par->codewd74);
/*
** Decode 4 MSB of second gain index using (7,4) Hamming code; parity bits in
** next 3 MSB of fsvq index.
*/
	    vgetbits(par->codewd74,par->gain_index[1],4,4);
	    vgetbits(&par->codewd74[4],par->fsvq_index[0],4,3);
	    berr_pos=sbc_dec(par->codewd74,7,4,&pmat74[0][0],syntab74);
	    par->gain_index[1]=vsetbits(par->gain_index[1],4,4,par->codewd74);
/*
...
*/
	    vgetbits(par->codewd74,par->gain_index[1],0,1);
	    vgetbits(&par->codewd74[1],par->gain_index[0],2,3);
	    vgetbits(&par->codewd74[4],par->fsvq_index[0],1,2);
	    vgetbits(&par->codewd74[6],par->jit_index,0,1);
	    berr_pos=sbc_dec(par->codewd74,7,4,&pmat74[0][0],syntab74);
	    par->gain_index[1]=vsetbits(par->gain_index[1],0,1,par->codewd74);
	    par->gain_index[0]=vsetbits(par->gain_index[0],2,3,&par->codewd74[1]);
	    par->jit_index = 1;
	}
    } /* if (par->uv_flag && !erase) */
...
/*
     ...
     int16_t, and returns a bit vector containing the pattern.  Conversely,
...
int16_t *vgetbits(int16_t *y, int16_t x, int16_t p, int16_t n)
{
    int16_t lsb=0x1; /* least significant bit mask */
    int16_t *retval=y;
...
int16_t sbc_dec(int16_t x[], int16_t n, int16_t k, const int16_t *pmat, const int16_t* syntab)
{
    int16_t bep=syntab[sbc_syn(x,n,k,pmat)];
    if (bep > -1)
	x[bep] ^= 0x1;
    return(bep);
}
