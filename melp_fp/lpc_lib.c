...
float lpc_schr(float *r, float *a, float *k, float *y1, float *y2)
{
    ...
    if (a != NULL)
    {
        (void)lpc_refl2pred(k, a);
    }
    return(alphap);
}

/* minimum LSP separation-- a global variable */
const float lsp_delta = 0.0;

/* private functions */
static float lsp_g(float x,float *c,int16_t p2);
static int16_t   lsp_roots(float *w,float **c,int16_t p2);
...
int16_t lpc_pred2lsp(float *a, float *w, float **c)
{
    ...
    c[0][LPC_ORD_HALF] = c[1][LPC_ORD_HALF] = 1.0;

    for(i=1; i <= LPC_ORD_HALF; i++)
    {
        c[0][LPC_ORD_HALF-i] = (a[i] + a[LPC_ORD+1-i] - c[0][LPC_ORD_HALF+1-i]);
        c[1][LPC_ORD_HALF-i] = c[1][LPC_ORD_HALF+1-i] + a[i] - a[LPC_ORD+1-i];
    }
    c[0][0] *= 0.5;
    c[1][0] *= 0.5;

    i = lsp_roots(w,c,LPC_ORD_HALF);
    ...
    (void)lpc_clamp(w, lsp_delta, LPC_ORD);
...
/* LPC_LSP2PRED
   ...
   Synopsis: lpc_lsp2pred(w,a)
      Input:
         w- the LSPs
         LPC_ORD - the predictor order
      Output:
         a- the predictor coefficients
   Reference:  Kabal and Ramachandran
*/
...
int16_t lpc_lsp2pred(float *w, float *a, float** f)
{
    ...
    for(i=LPC_ORD_HALF; i > 0; i--)
    {
        f[0][i] += f[0][i-1];
        f[1][i] -= f[1][i-1];

        a[i] = 0.50*(f[0][i]+f[1][i]);
        a[LPC_ORD+1-i] = 0.50*(f[0][i]-f[1][i]);
    }
...
static int16_t lsp_roots(float *w,float **c,int16_t p2)
{
    ...
    for(k=1,x = 1.0-DELTA; x > -DELTA-1.0; x -= DELTA)
    {
        ...
        if (g0*(g1 = lsp_g(x,ptr,p2)) <= 0.0)
        {
            ...
            /* Evaluate the LSF */
            w[k] = (float)acos((double)x) * INV_PI;
