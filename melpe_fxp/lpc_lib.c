...
typedef struct _lpc_lib_context {
	BOOLEAN firstTime;
	int16_t lsp_cos[DFTLENGTH];	/* cosine table */
	int16_t default_w, default_w0;
} lpc_lib_context;

/* Prototype */

static void lsp_to_freq(lpc_lib_context* lpc, int16_t lsp[], int16_t freq[], int16_t order);
			...
			for (i = 0; i < order - 1; i++) {
				...
				if (d < delta) {
					...
					if (i == 0  && (lsp[i] < delta)) 
					{
						...
						if (i > 0) {
							temp = melpe_sub(lsp[i], lsp[i - 1]);
							if (temp < delta) {
								step1 = 0;
							} else {
								if (temp < melpe_shl(delta,1))
									step1 =  melpe_shr(melpe_sub (temp, delta), 1);
							}
						}
					}

					if (i == (order - 2) && (lsp[i+1] >  melpe_sub(ONE_Q15,delta)))
					{
						step2 = melpe_shr(melpe_sub(ONE_Q15, lsp[i + 1]), 1);
					} else {
						if (i < (order - 2)) 
						{
							temp =  melpe_sub(lsp[i + 2], lsp[i + 1]);
							if (temp < delta) {
								step2 = 0;
							} else {
								if (temp < melpe_shl(delta,1))
									step2 = melpe_shr(melpe_sub(temp,delta),1);
							}
						}
					}
...
int16_t lpc_pred2lsp(void* lpc_handle, int16_t lpc[], int16_t lsf[], int16_t order)
{
	register int16_t i;
	lpc_lib_context* l_ctx = (lpc_lib_context*) lpc_handle;
	...
	lsp_to_freq(l_ctx, p_cof, p_freq, order);
	lsp_to_freq(l_ctx, q_cof, q_freq, order);
...
static void lsp_to_freq(lpc_lib_context* lpc, int16_t lsp[], int16_t freq[], int16_t order)
{
	...
	if (lpc->firstTime) {
		...
		for (i = 0; i <= DFTLENGTH_D4; i++) {
			lpc->lsp_cos[i] = cos_fxp(temp1);
			lpc->lsp_cos[i + DFTLENGTH_D2] = melpe_negate(lpc->lsp_cos[i]);
			temp1 = melpe_add(temp1, DFTLENGTH_D4);
		}
		temp1 = DFTLENGTH_D4;
		temp2 = DFTLENGTH_D4;
		for (i = 0; i < DFTLENGTH_D4; i++) {
			lpc->lsp_cos[temp1] = melpe_negate(lpc->lsp_cos[temp2]);
			lpc->lsp_cos[temp1 + DFTLENGTH_D2] = lpc->lsp_cos[temp2];
		...
		/* (1./p2) in Q15 */
		lpc->default_w = melpe_divide_s(ONE_Q11, melpe_shl(order, 10));
		/* freq[0] = (0.5/p2) in Q15 */
		lpc->default_w0 = melpe_shr(lpc->default_w, 1);
	...
	for (i = 0; i <= DFTLENGTH_D2; i++) {
		...
		for (j = melpe_sub(p2, 1); j >= 0; j--) {
			L_temp1 = melpe_L_shr(melpe_L_mult(lsp[j], lpc->lsp_cos[p_cos]), 1);
		...
		*/
		mag[0]=mag[1]; s_mag[0]=s_mag[1];
		mag[1]=mag[2]; s_mag[1]=s_mag[2];
	}
	...
	if (count != p2) {
		/* use default values */
		freq[0] = lpc->default_w0;
		for (i = 1; i < p2; i++)
			freq[i] = melpe_add(freq[i - 1], lpc->default_w);
	}
}
...
/* ======================================= */
/* lpc_init(): this is to workaround the TI
 * compiler feature to omit static and global variables zeroing */
/* ======================================= */
void* lpc_init(void* context)
{
	lpc_lib_context* lpc = (lpc_lib_context*) context;
	if (!lpc) lpc = (lpc_lib_context*) malloc(sizeof(lpc_lib_context));
	if (!lpc) return 0;
	memset(lpc, 0x00, sizeof(lpc_lib_context));
	lpc->firstTime = TRUE;
	return (void*) lpc;
}
