...
void rand_num(int16_t output[], int16_t amplitude, int16_t npts, uint32_t* seed_ptr)
{
	register int16_t i;
	int16_t temp;
	...
	for (i = 0; i < npts; i++) {
		...
		/* -0.5 <= temp < 0.5 */
		temp = melpe_sub(rand_minstdgen(seed_ptr), X05_Q15);
...
int16_t rand_minstdgen(uint32_t* seed_ptr)
{
	uint16_t x0 = melpe_extract_l(*seed_ptr);	/* 16 LSBs OF SEED */
	uint16_t x1 = melpe_extract_h(*seed_ptr);	/* 16 MSBs OF SEED */
	uint32_t p, q;		/* MSW, LSW OF PRODUCT */
	uint32_t L_temp1, L_temp2, L_temp3;

	/*----------------------------------------------------------------------*/
	/* COMPUTE THE PRODUCT (A * seed) USING CROSS MULTIPLICATION OF         */
