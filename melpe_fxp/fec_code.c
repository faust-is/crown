...
typedef struct _fec_code_context {
	int16_t codewd74[7];
	int16_t codewd84[8];
	int16_t codewd_13x9[13];
} fec_code_context;
...
void fec_code(void* fec_code_handle, struct quant_param *qpar)
{
	fec_code_context* f = (fec_code_context*) fec_code_handle;
	...
	if (qpar->uv_flag[0]) {
		...
		vgetbits(f->codewd84, qpar->msvq_index[0], 6, 4);
		sbc_enc(f->codewd84, 8, 4, &pmat84[0][0]);
		vsetbits(qpar->bpvc_index, 3, 4, &f->codewd84[4]);

		/* Code 3 LSB of first vq stage index using (7,4) Hamming code;       */
		/* parity bits in 3 MSB of fsvq index.                                */

		vgetbits(f->codewd74, qpar->msvq_index[0], 2, 3);
		f->codewd74[3] = 0;
		sbc_enc(f->codewd74, 7, 4, &pmat74[0][0]);
		vsetbits(&(qpar->fsvq_index), 7, 3, &f->codewd74[4]);

		/* Code 4 MSB of second gain index using (7,4) Hamming code; parity   */
		/* bits in next 3 MSB of fsvq index.                                  */

		vgetbits(f->codewd74, qpar->gain_index[1], 4, 4);
		sbc_enc(f->codewd74, 7, 4, &pmat74[0][0]);
		vsetbits(&(qpar->fsvq_index), 4, 3, &f->codewd74[4]);

		/* Code LSB of second gain index, first gain index using (7,4)        */
		/* Hamming code; parity bits in 2 LSB of fsvq index, jitter index     */
		/* bit.                                                               */

		vgetbits(f->codewd74, qpar->gain_index[1], 0, 1);
		vgetbits(&f->codewd74[1], qpar->gain_index[0], 2, 3);
		sbc_enc(f->codewd74, 7, 4, &pmat74[0][0]);
		vsetbits(&(qpar->fsvq_index), 1, 2, &f->codewd74[4]);
		vsetbits(qpar->jit_index, 0, 1, &f->codewd74[6]);
	}
...
void low_rate_fec_code(void* fec_code_handle, struct quant_param *par)
{
	fec_code_context* f = (fec_code_context*) fec_code_handle;
	...
	if (par->uv_flag[0] && par->uv_flag[1] && par->uv_flag[2]) {
		...
		/* for 4 MSB of Gain */
		vgetbits(f->codewd84, par->gain_index[0], 9, 4);
		sbc_enc(f->codewd84, 8, 4, &pmat84[0][0]);
		vsetbits(&(par->fs_index), 7, 4, &f->codewd84[4]);

		/* for next 4 MSB of Gain */
		vgetbits(f->codewd84, par->gain_index[0], 5, 4);
		sbc_enc(f->codewd84, 8, 4, &pmat84[0][0]);
		vsetbits(&(par->fs_index), 3, 4, &f->codewd84[4]);

		/* for 3 LSB of Gain */
		vgetbits(f->codewd74, par->gain_index[0], 1, 2);
		f->codewd74[2] = f->codewd74[3] = 0;
		sbc_enc(f->codewd74, 7, 4, &pmat74[0][0]);
		vsetbits(&(par->bpvc_index[0]), 1, 2, &f->codewd74[4]);
		vsetbits(&(par->jit_index[0]), 0, 1, &f->codewd74[6]);

		/* for LSPs of 1st frame */
		vgetbits(&f->codewd_13x9[4], par->lsf_index[0][0], 8, 9);
		crc4_enc(f->codewd_13x9, 9);
		vsetbits(&(par->lsf_index[0][1]), 3, 4, &f->codewd_13x9[0]);

		/* for LSPs of 2nd frame */
		vgetbits(&f->codewd_13x9[4], par->lsf_index[1][0], 8, 9);
		crc4_enc(f->codewd_13x9, 9);
		vsetbits(&(par->lsf_index[1][1]), 3, 4, &f->codewd_13x9[0]);

		/* for LSPs of 3rd frame */
		vgetbits(&f->codewd_13x9[4], par->lsf_index[2][0], 8, 9);
		crc4_enc(f->codewd_13x9, 9);
		vsetbits(&(par->lsf_index[2][1]), 3, 4, &f->codewd_13x9[0]);
	}
}

int16_t fec_decode(void* fec_code_handle, struct quant_param *qpar, int16_t erase)
{
	int16_t berr_pos;
	fec_code_context* f = (fec_code_context*) fec_code_handle;
	...
	if (qpar->uv_flag[0] && !erase) {
		...
		vgetbits(f->codewd84, qpar->msvq_index[0], 6, 4);
		vgetbits(&f->codewd84[4], qpar->bpvc_index[0], 3, 4);
		berr_pos = sbc_dec(f->codewd84, 8, 4, &pmat84[0][0], syntab84);
		erase |= (berr_pos == BEP_UNCORR);
		vsetbits(qpar->msvq_index, 6, 4, f->codewd84);
		qpar->bpvc_index[0] = 0;
		...
		if (!erase) {
			...
			vgetbits(f->codewd74, qpar->msvq_index[0], 2, 3);
			f->codewd74[3] = 0;
			vgetbits(&f->codewd74[4], qpar->fsvq_index, 7, 3);
			sbc_dec(f->codewd74, 7, 4, &pmat74[0][0], syntab74);
			vsetbits(qpar->msvq_index, 2, 3, f->codewd74);

			/* Decode 4 MSB of second gain index using (7,4) Hamming code;    */
			/* parity bits in next 3 MSB of fsvq index.                       */

			vgetbits(f->codewd74, qpar->gain_index[1], 4, 4);
			vgetbits(&f->codewd74[4], qpar->fsvq_index, 4, 3);
			sbc_dec(f->codewd74, 7, 4, &pmat74[0][0], syntab74);
			vsetbits(&(qpar->gain_index[1]), 4, 4, f->codewd74);
			...
			vgetbits(f->codewd74, qpar->gain_index[1], 0, 1);
			vgetbits(&f->codewd74[1], qpar->gain_index[0], 2, 3);
			vgetbits(&f->codewd74[4], qpar->fsvq_index, 1, 2);
			vgetbits(&f->codewd74[6], qpar->jit_index[0], 0, 1);
			sbc_dec(f->codewd74, 7, 4, &pmat74[0][0], syntab74);
			vsetbits(&(qpar->gain_index[1]), 0, 1, f->codewd74);
			vsetbits(qpar->gain_index, 2, 3, &f->codewd74[1]);
			qpar->jit_index[0] = 1;
		}
	}
	return (erase);
...
int16_t low_rate_fec_decode(void* fec_code_handle, struct quant_param * qpar, int16_t erase,
			      int16_t lsp_check[])
{
	int16_t berr_pos;
	fec_code_context* f = (fec_code_context*) fec_code_handle;
	...
	if (qpar->uv_flag[0] && qpar->uv_flag[1] && qpar->uv_flag[2]) {
		/* for 4 MSB of Gain */
		vgetbits(f->codewd84, qpar->gain_index[0], 9, 4);
		vgetbits(&f->codewd84[4], qpar->fs_index, 7, 4);
		berr_pos = sbc_dec(f->codewd84, 8, 4, &pmat84[0][0], syntab84);
		erase |= berr_pos == BEP_UNCORR;
		vsetbits(qpar->gain_index, 9, 4, f->codewd84);

		/* for next 4 MSB of Gain */
		vgetbits(f->codewd84, qpar->gain_index[0], 5, 4);
		vgetbits(&f->codewd84[4], qpar->fs_index, 3, 4);
		berr_pos = sbc_dec(f->codewd84, 8, 4, &pmat84[0][0], syntab84);
		erase |= berr_pos == BEP_UNCORR;
		vsetbits(qpar->gain_index, 5, 4, f->codewd84);

		if (!erase) {

			/* for 2 LSB of Gain */
			vgetbits(f->codewd74, qpar->gain_index[0], 1, 2);
			f->codewd74[2] = f->codewd74[3] = 0;
			vgetbits(&f->codewd74[4], qpar->bpvc_index[0], 1, 2);
			vgetbits(&f->codewd74[6], qpar->jit_index[0], 0, 1);
			sbc_dec(f->codewd74, 7, 4, &pmat74[0][0], syntab74);
			vsetbits(qpar->gain_index, 1, 2, f->codewd74);

			/* for LSPs of 1st frame */
			vgetbits(&f->codewd_13x9[4], qpar->lsf_index[0][0], 8, 9);
			vgetbits(&f->codewd_13x9[0], qpar->lsf_index[0][1], 3, 4);
			lsp_check[0] = crc4_dec(f->codewd_13x9, 13);

			/* for LSPs of 1st frame */
			vgetbits(&f->codewd_13x9[4], qpar->lsf_index[1][0], 8, 9);
			vgetbits(&f->codewd_13x9[0], qpar->lsf_index[1][1], 3, 4);
			lsp_check[1] = crc4_dec(f->codewd_13x9, 13);

			/* for LSPs of 1st frame */
			vgetbits(&f->codewd_13x9[4], qpar->lsf_index[2][0], 8, 9);
			vgetbits(&f->codewd_13x9[0], qpar->lsf_index[2][1], 3, 4);
			lsp_check[2] = crc4_dec(f->codewd_13x9, 13);
		}
	}
	return (erase);
}
...
/* ======================================= */
/* fec_code_init(): this is to workaround the TI
 * compiler feature to omit static and global variables zeroing */
/* ======================================= */
void* fec_code_init(void* handle)
{
	fec_code_context* f = (fec_code_context*) handle;
	if (!f) f = (fec_code_context*) malloc(sizeof(fec_code_context));
	if (!f) return 0;
	memset(f, 0x00, sizeof(fec_code_context));
	return (void*) f;
}
