...
typedef struct _qnt12_context {
	int16_t qplsp[LPC_ORD];	/* Q15 */
	int16_t prev_fsmag[NUM_HARM];
	int16_t prev_pitch;	/* Q12 */
	int16_t prev_qpitch;	/* Q12 */
	BOOLEAN prev_uv;
	BOOLEAN prev_uv_flag;
} qnt12_context;
...
void pitch_vq(void* melpe_handle, struct melp_param *par)
{
	...
	qnt12_context* q = m->qnt12_handle;
	...
	/* ---- calculate delta ---- */
	for (i = 0; i < NF; i++) {
		if (q->prev_uv_flag || par[i].uv_flag) {
			deltp[i] = 0;
			deltw[i] = 0;
		} else {
			deltp[i] = melpe_sub(target[i], q->prev_pitch);
			deltw[i] = DELTA_PITCH_WEIGHT_Q0;
		}
		q->prev_pitch = target[i];
		q->prev_uv_flag = par[i].uv_flag;
	}
	...
	if (cnt == 0) {
		...
		q->prev_qpitch = LOG_UV_PITCH_Q12;

	} else if (cnt == 1) {

		for (i = 0; i < NF; i++) {
			if (!par[i].uv_flag) {
				quant_u(&target[i], &(g->quant_par.pitch_index),
					PIT_QLO_Q12, PIT_QUP_Q12, PIT_QLEV_M1,
					PIT_QLEV_M1_Q8, TRUE, 7);
				quant_u_dec(g->quant_par.pitch_index,
		...
		q->prev_qpitch = par[NF - 1].pitch;	/* Q12 */
		...
		for (i = 0; i < PITCH_VQ_CAND; i++) {
			...
			dcb[temp1] = melpe_sub(codebook[temp2], q->prev_qpitch);	/* Q12 */
		...
		if (par[NF - 1].uv_flag)
			...
			q->prev_qpitch = codebook[pitch_index * NF + NF - 1];	/* Q12 */
		...
		g->quant_par.pitch_index = pitch_index;
	}
}
...
void gain_vq(void* melpe_handle, struct melp_param *par)
{
	...
	g->quant_par.gain_index[0] = index;
}
...
void quant_bp(void* melpe_handle, struct melp_param *par, int16_t num_frames)
{
	...
	for (i = 0; i < num_frames; i++) {
		par[i].uv_flag = q_bpvc(par[i].bpvc, &(g->quant_par.bpvc_index[i]),
					NUM_BANDS);
		g->quant_par.bpvc_index[i] = bp_index_map[g->quant_par.bpvc_index[i]];
	}
}
...
BOOLEAN lspStable(int16_t lsp[], int16_t order)
{
	...
	if (lsp[order - 1] < lsp[order - 2])
		...
		/* Warning message moved from lspSort() to lspStable(). */
...
void lsf_vq(void* melpe_handle, struct melp_param *par)
{
	...
	case 7:		/* 111, all frames are NOT voiced ---- */
		lspVQ(lsp[0], wgt[0], lsp[0], lsp_uv_9, 1, melp_uv_cb_size,
		      g->quant_par.lsf_index[0], LPC_ORD, FALSE);
		lspVQ(lsp[1], wgt[1], lsp[1], lsp_uv_9, 1, melp_uv_cb_size,
		      g->quant_par.lsf_index[1], LPC_ORD, FALSE);
		lspVQ(lsp[2], wgt[2], lsp[2], lsp_uv_9, 1, melp_uv_cb_size,
		      g->quant_par.lsf_index[2], LPC_ORD, FALSE);
		break;
	case 6:		/* 110 */
		lspVQ(lsp[0], wgt[0], lsp[0], lsp_uv_9, 1, melp_uv_cb_size,
		      g->quant_par.lsf_index[0], LPC_ORD, FALSE);
	...
	default:
		...
		for (k = 0; k < LSP_INP_CAND; k++) {
			for (i = 0; i < 16; i++) {
				...
				for (j = 0; j < LPC_ORD; j++) {
					...
					acc = melpe_L_mult(intfact, q->qplsp[j]);	/* Q30 */
					...
					bcc = melpe_L_mult(intfact, q->qplsp[j]);
		...
		v_equ(g->quant_par.lsf_index[0], &(lsp_index_cand[cand * tos]),
		      tos);
		g->quant_par.lsf_index[1][0] = inp_index_cand;
		...
		if (uv_config == 1)	/* if (!uv1 && !uv2 && uv3) */
			lspVQ(res, mwgt, res, res256x64x64x64, 4, res_cb_size,
			      g->quant_par.lsf_index[2], 2 * LPC_ORD, FALSE);
		else
...
void quant_jitter(void* melpe_handle, struct melp_param *par)
{
	...
	g->quant_par.jit_index[0] = flag_jitter;
}
...
void quant_fsmag(void* melpe_handle, struct melp_param *par)
{
	...
	for (i = 0; i < NF; i++) {
		if (par[i].uv_flag)
			...
			window_Q(par[i].fs_mag, g->w_fs, par[i].fs_mag, NUM_HARM,
				 14);
			last = i;
			count++;
		}
	...
	if (count > 0)
		vq_enc(fsvq_cb, par[last].fs_mag, FS_LEVELS, NUM_HARM, qmag,
		       &g->quant_par.fs_index);

	if (count > 1) {
		if (q->prev_uv || par[0].uv_flag) {
			for (i = 0; i <= last; i++) {
				if (!par[i].uv_flag)
			...
			if (par[1].uv_flag) {	/* V VUV */
				v_equ(par[0].fs_mag, q->prev_fsmag, NUM_HARM);
	...
	q->prev_uv = par[NF - 1].uv_flag;
	if (q->prev_uv)
		fill(q->prev_fsmag, ONE_Q13, NUM_HARM);
	else
		v_equ(q->prev_fsmag, par[NF - 1].fs_mag, NUM_HARM);
}

/* ======================================= */
/* qnt12_init(): this is to workaround the TI
...
void* qnt12_init(void* context)
{
	...
	qnt12_context* q = (qnt12_context*) context;
	if (!q) q = (qnt12_context*) malloc(sizeof(qnt12_context));
	if (!q) return 0;
	memset(q, 0x00, sizeof(qnt12_context));
	
	q->prev_uv_flag = TRUE;
	q->prev_pitch = LOG_UV_PITCH_Q12;
	q->prev_qpitch = LOG_UV_PITCH_Q12;
	q->prev_uv = TRUE;
	...
	for (i = 0; i < LPC_ORD; i++) {
		...
		q->qplsp[i] = melpe_divide_s(temp1, temp2);
