...
typedef struct _pit_context {
	int16_t lpres_delin[LPF_ORD];
	int16_t lpres_delout[LPF_ORD];
	int16_t sigbuf[LPF_ORD + PITCH_FR];
	int16_t good_pitch[NF];
} pit_context;	
...
int16_t p_avg_update(void* pit_handle, int16_t pitch, int16_t pcorr, int16_t pthresh)
{
	register int16_t i;
	int16_t pitch_avg, temp;
	pit_context* p = (pit_context*) pit_handle;
	...
	if (pcorr > pthresh) {
		...
		/* VSHIFT: Original code:
		    v_equ(p->good_pitch, &(p->good_pitch[1]), NF - 1);
		*/
#ifndef POSIX_TI_EMULATION
		#pragma MUST_ITERATE(2,2)
#endif
		for (i=0; i< NF-1; i++) p->good_pitch[i]=p->good_pitch[i+1]; 
		
		p->good_pitch[NF - 1] = pitch;
	} else {		/* Otherwise decay good pitch array to default value */
		for (i = 0; i < NF; i++) {
			/*      p->good_pitch[i] =
			   (PDECAY*p->good_pitch[i]) +((1.0-PDECAY)*DEFAULT_PITCH); */
			temp = melpe_mult(PDECAY_Q15, p->good_pitch[i]);
			p->good_pitch[i] = melpe_add(temp, PDECAY_PITCH_Q7);
		}
	}

	/* Pitch_avg = median of pitch values */
	pitch_avg = median3(p->good_pitch);
...
int16_t pitch_ana(void* pit_handle, int16_t speech[], int16_t resid[],
		    ...
		    int16_t * pcorr2)
{
	...
	v_equ(&p->sigbuf[LPF_ORD_SOS], &resid[-PITCHMAX], PITCH_FR);

	for (section = 0; section < LPF_ORD / 2; section++) {
		iir_2nd_s(&p->sigbuf[LPF_ORD_SOS], &lpf_den[section * 3],
			  &lpf_num[section * 3], &p->sigbuf[LPF_ORD_SOS],
			  &p->lpres_delin[section * 2], &p->lpres_delout[section * 2],
			  FRAME);
		/* save delay buffers for the next overlapping frame */
		for (i = (int16_t) (section * 2);
		     i < (int16_t) (section * 2 + 2); i++) {
			temp_delin[i] = p->lpres_delin[i];
			temp_delout[i] = p->lpres_delout[i];
		}
		iir_2nd_s(&p->sigbuf[LPF_ORD_SOS + FRAME], &lpf_den[section * 3],
			  &lpf_num[section * 3], &p->sigbuf[LPF_ORD_SOS + FRAME],
			  &p->lpres_delin[section * 2], &p->lpres_delout[section * 2],
			  PITCH_FR - FRAME);
		/* restore delay buffers for the next overlapping frame */
		for (i = (int16_t) (section * 2);
		     i < (int16_t) (section * 2 + 2); i++) {
			p->lpres_delin[i] = temp_delin[i];
			p->lpres_delout[i] = temp_delout[i];
		}
	}

	/* Scale lowpass residual for pitch correlations */
	f_pitch_scale(&p->sigbuf[LPF_ORD_SOS], &p->sigbuf[LPF_ORD_SOS], PITCH_FR);

	/* Perform local search ar_ound pitch estimate */
	temp =
	    frac_pch(&p->sigbuf[LPF_ORD_SOS + (PITCH_FR / 2)], &pcorr, pitch_est,
	...
	if (pcorr < PCORR_THR) {
		...
		v_equ(&p->sigbuf[LPF_ORD], &speech[-PITCHMAX], PITCH_FR);

		/* Scale speech for pitch correlations */
		f_pitch_scale(&p->sigbuf[LPF_ORD_SOS], &p->sigbuf[LPF_ORD_SOS],
			      PITCH_FR);

		temp =
		    frac_pch(&p->sigbuf[LPF_ORD + (PITCH_FR / 2)], &pcorr,
		...
		if (pcorr < UVMAX)	/* If correlation still too low, use average pitch */
			...
			if (temp > LONG_PITCH)
				...
				temp2 = PDOUBLE3;
			pitch =
			    double_chk(&p->sigbuf[LPF_ORD + (PITCH_FR / 2)],
		...
		if (temp > LONG_PITCH)
			...
			temp2 = PDOUBLE1;
		pitch = double_chk(&p->sigbuf[LPF_ORD + (PITCH_FR / 2)], &pcorr,
...
/* ======================================= */
/* pit_lib_init(): this is to workaround the TI
 * compiler feature to omit static and global variables zeroing */
/* ======================================= */
void* pit_lib_init(void* context)
{
	pit_context* p = (pit_context*) context;
	if (!p) p = (pit_context*) malloc(sizeof(pit_context));
	if (!p) return 0;
	memset(p, 0x00, sizeof(pit_context));
	
	fill(p->good_pitch, DEFAULT_PITCH_Q7, NF);
