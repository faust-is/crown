...
typedef struct _chn_context {
	/* Define bit buffer */
	unsigned char bit_buffer[NUM_CH_BITS];
	int16_t sync_bit;	/* sync bit */
	/* moved from low_rate_chn_read() */
	int16_t prev_uv;
	int16_t prev_fsmag[NUM_HARM];
	int16_t qplsp[LPC_ORD], prev_gain[2 * NF * NUM_GAINFR];
	int16_t q_gain_dec_prev_gain; /* q_gain_dec */
	int16_t prev_gain_err; /* q_gain_dec */
	int16_t firstTime;
} chn_context;
	
int16_t parity(int16_t x, int16_t leng);

void melp_chn_write(void* melpe_handle, struct quant_param *qpar, unsigned char chbuf[])
{
	melpe_context* m = (melpe_context*) melpe_handle;
	global_context* g = m->global_handle;
	chn_context* c = m->chn_handle;
	...
	fec_code(m->fec_code_handle, qpar);

	/* Fill bit buffer */
	bit_ptr = c->bit_buffer;
	bit_cntr = 0;

	pack_code(qpar->gain_index[1], &bit_ptr, &bit_cntr, 5, 1);

	/* Toggle and write sync bit */
	c->sync_bit = melpe_sub(1, c->sync_bit);
	pack_code(c->sync_bit, &bit_ptr, &bit_cntr, 1, 1);
	...
	qpar->chptr = chbuf;
	qpar->chbit = 0;
	for (i = 0; i < g->bitNum; i++) {
		pack_code(c->bit_buffer[bit_order[i]], &qpar->chptr, &qpar->chbit,
			  1, g->chwordsize);
...
BOOLEAN melp_chn_read(void* melpe_handle, struct quant_param *qpar, struct melp_param *par,
		      struct melp_param *prev_par, unsigned char chbuf[])
{
	...
	for (i = 0; i < g->bitNum; i++) {
		erase |= unpack_code(&(qpar->chptr), &(qpar->chbit), &index, 1,
				     g->chwordsize, ERASE_MASK);
		c->bit_buffer[bit_order[i]] = (unsigned char)index;
		bit_ptr++;
	}
	...
	erase = (BOOLEAN) fec_decode(m->fec_code_handle, qpar, erase);
	...
	if (erase) {		/* Erasure: frame repeat */
		*par = *prev_par;
		...
		q_gain_dec(par->gain, qpar->gain_index, GN_QLO_Q8, GN_QUP_Q8,
			   GN_QLEV_M1_Q10, 5, &c->q_gain_dec_prev_gain, &c->prev_gain_err);
...
**
*****************************************************************************/
void low_rate_chn_write(void* melpe_handle, struct quant_param *qpar)
{
	...
	low_rate_fec_code(m->fec_code_handle, qpar);
	...
	qpar->chbit = 0;
	for (i = 0; i < g->bitNum; i++) {
		pack_code(c->bit_buffer[i], &qpar->chptr, &qpar->chbit, 1,
			  g->chwordsize);
		if (i == 0)
...
**
*****************************************************************************/
int16_t low_rate_chn_read(void* melpe_handle, struct quant_param *qpar, struct melp_param *par,
			    struct melp_param *prev_par)
{
	...
	/* In previous versions we use unweighted_fsmag[] and c->prev_fsmag[] to     */
	...
	if (c->firstTime) {	/* Initialization */
		temp2 = melpe_shl(LPC_ORD, 10);	/* Q10 */
		temp1 = X08_Q10;	/* Q10 */
		for (i = 0; i < LPC_ORD; i++) {
			/*      c->qplsp[i] = (i + 1)*0.8/LPC_ORD; */
			c->qplsp[i] = melpe_divide_s(temp1, temp2);
			temp1 = melpe_add(temp1, X08_Q10);
		}

		fill(c->prev_gain, 2560, 2 * NF * NUM_GAINFR);
		fill(c->prev_fsmag, ONE_Q13, NUM_HARM);
	...
	for (i = 0; i < g->bitNum; i++) {
		erase |= unpack_code(&qpar->chptr, &qpar->chbit, &index, 1,
				     g->chwordsize, ERASE_MASK);
		c->bit_buffer[i] = (unsigned char)index;
		bit_ptr++;
	}
	...
	/* ====== FEC decoding ====== */
	erase = low_rate_fec_decode(m->fec_code_handle, qpar, erase, lsp_check);
	...
	if (flag_dec_lsp) {
		if ((uv1 == 1) && (uv2 == 1) && (cuv == 1)) {
			...
			for (j = 0; j < LPC_ORD; j++) {
				/*      ilsp1[j] = inpCoef[i][j] * c->qplsp[j] +
				   (1.0 - inpCoef[i][j]) * par[2].lsf[j]; */
				intfact = inpCoef[i][j];	/* Q14 */
				L_acc = melpe_L_mult(intfact, c->qplsp[j]);	/* Q14 */
				...
				/*      ilsp2[j] = inpCoef[i][j+10] * c->qplsp[j] +
		...
		/* ---- Save for next frame ---- */
		v_equ(c->qplsp, par[NF - 1].lsf, LPC_ORD);
	} else {
		for (i = 0; i < NF; i++)
	...
	if (cnt > 1) {
		if (c->prev_uv) {	/* U {UVV,VUV,VVU,VVV} */
			for (i = 0; i < last; i++)
				if (!par[i].uv_flag)
	...
	c->prev_uv = par[NF - 1].uv_flag;
	if (par[NF - 1].uv_flag) {
		fill(c->prev_fsmag, ONE_Q13, NUM_HARM);
		window_Q(c->prev_fsmag, g->w_fs, c->prev_fsmag, NUM_HARM, 14);
	} else
		v_equ(c->prev_fsmag, par[NF - 1].fs_mag, NUM_HARM);
	...
	if (flag_parity) {
		...
		for (i = 0; i < 2 * NF * NUM_GAINFR; i++)
			L_sum1 = melpe_L_add(L_sum1, melpe_L_deposit_l(c->prev_gain[i]));
	...
	for (i = 0; i < NF * NUM_GAINFR; i++)
		c->prev_gain[i] = c->prev_gain[i + NF * NUM_GAINFR];
	for (i = 0; i < NF; i++) {
		for (j = 0; j < NUM_GAINFR; j++)
			c->prev_gain[NF * NUM_GAINFR + i * NUM_GAINFR + j] =
			    par[i].gain[j];
	}
...
/* ======================================= */
/* melp_chn_init(): this is to workaround the TI
 * compiler feature to omit static and global variables zeroing */
/* ======================================= */
void* melp_chn_init(void* context)
{

	chn_context* c = (chn_context*) context;
	if (!c) c = (chn_context*) malloc(sizeof(chn_context));
	if (!c) return 0;
	memset(c, 0x00, sizeof(chn_context));
	c->prev_uv = 1;
	c->firstTime = TRUE;
	return (void*) c;
}
