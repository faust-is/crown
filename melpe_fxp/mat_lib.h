...
static inline int16_t *v_move_aligned(int16_t* vec1, int16_t* vec2, int16_t n)
{
#ifdef POSIX_TI_EMULATION
	memmove(vec1, vec2, n << 1);
#else
	/* Optimized memory move */
	/* Constraints: n % 8 == 0, n >= 32, v1 % 8 ==0, v2 % 8 ==0 */
	_DSP_blk_move(vec2, vec1, n);
#endif
	return (vec1);
}
