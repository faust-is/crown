...
typedef struct _postfilt_context 
{
	int16_t mem1[LPC_ORD];
	int16_t mem2[LPC_ORD];
	int16_t postfilt_hpm;	/* Q0 */
	int16_t postfilt_gain;	/* Q14 */
	int16_t alphaipFIR[LPC_ORD];	/* Q12 */
	int16_t alphaipIIR[LPC_ORD];
	int16_t window[SMOOTH_LEN];	/* Q15 */
	int16_t hpf60_delin[2];	/* Q13 */
	int16_t hpf60_delout_hi[2];
	int16_t hpf60_delout_lo[2];
	int16_t lpf3500_delin[2];	/* Q13 */
	int16_t lpf3500_delout_hi[2];
	int16_t lpf3500_delout_lo[2];
} postfilt_context;

/* ===== Prototypes ===== */
static void hpf60(postfilt_context* p, int16_t speech[]);
static void lpf3500(postfilt_context* p, int16_t speech[]);
...
void postfilt(void* postfilt_handle, int16_t speech[], int16_t prev_lsf[], int16_t cur_lsf[])
{
	register int16_t i, j, k;
	postfilt_context* p = (postfilt_context*) postfilt_handle;
	...
	for (i = 0; i < SYN_SUBNUM; i++) {
		...
		for (j = 0; j < SYN_SUBFRAME; j++) {
			temp = melpe_mult(emph, p->postfilt_hpm);	/* Q0 */
			p->postfilt_hpm = speech[i * SYN_SUBFRAME + j];
			synhp[j] = melpe_sub(p->postfilt_hpm, temp);	/* Q0 */
		}
		...
		if (i == 0) {
			v_equ(mem1old, p->mem1, LPC_ORD);
			v_equ(mem2old, p->mem2, LPC_ORD);

			for (j = 0; j < SMOOTH_LEN; j++) {
				L_sum = 0;
				for (k = 0; k < LPC_ORD; k++) {
					L_temp = melpe_L_mult(mem1old[k], p->alphaipFIR[k]);	/* Q13 */
				...
				for (k = 0; k < LPC_ORD; k++) {
					L_temp = melpe_L_mult(mem2old[k], p->alphaipIIR[k]);	/* Q13 */
				...
				mem2old[0] = temp1;	/* Q0 */
				temp = melpe_sub(ONE_Q15, p->window[j]);
				...
				L_temp = melpe_L_mult(p->postfilt_gain, temp1);	/* Q15 */
		...
		temp2 = BETA;
		for (j = 0; j < LPC_ORD; j++) {
			p->alphaipFIR[j] = melpe_mult(synLPC[j], temp1);	/* Q12 */
			p->alphaipIIR[j] = melpe_mult(synLPC[j], temp2);
		...
		for (j = 0; j < SYN_SUBFRAME; j++) {
			L_sum = 0;
			for (k = 0; k < LPC_ORD; k++) {
				L_temp = melpe_L_mult(p->mem1[k], p->alphaipFIR[k]);	/* Q13 */
				L_sum = melpe_L_add(L_sum, L_temp);	/* Q13 */
			}
			for (k = LPC_ORD - 1; k > 0; k--)
				p->mem1[k] = p->mem1[k - 1];
			...
			for (k = 0; k < LPC_ORD; k++) {
				L_temp = melpe_L_mult(p->mem2[k], p->alphaipIIR[k]);	/* Q13 */
				L_sum = melpe_L_sub(L_sum, L_temp);	/* Q13 */
			}
			for (k = LPC_ORD - 1; k > 0; k--)
				p->mem2[k] = p->mem2[k - 1];
			L_sum = melpe_L_shr(L_sum, 13);	/* Q0 */
			p->mem2[0] = melpe_extract_l(L_sum);	/* Q0 */
	...
	/* postfilt_gain ranges between 0.41 and 1.10.  It is difficult to estimate the    */
	/* absolute upper and lower bounds for postfilt_gain, because we only know emph    */
	...
	/* will simply use Q14 for postfilt_gain and apply the necessary truncations.      */
	...
	if (op_shift >= -22) {
		...
		/* "bullet-proof", we can saturate postfilt_gain computed below.               */

		p->postfilt_gain = melpe_shl(temp, temp_shift);	/* Q14 */
	} else
		p->postfilt_gain = 0;

	for (i = 0; i < FRAME; i++) {
		L_temp = melpe_L_mult(p->postfilt_gain, speech[i]);	/* Q15 */
		speech[i] = melpe_extract_l(melpe_L_shr(L_temp, 15));	/* Q0 */
	}

	/* The add() used for pos[i] below should not result in saturations.      */
	/* pos[] is multiplied by p->window[], while when we computed nokori[] it    */
	/* was multiplied by (1 - p->window[]).  postfilt_gain multiplied by the filter       */
	/* output does not seem to saturate.                                      */

	for (i = 0; i < SMOOTH_LEN; i++) {
		temp = melpe_mult(speech[i], p->window[i]);	/* Q0 */
	...
	/*     filter and move the postfilt_gain multiplication out of the filters.        */
	/* (3) Both filters contribute a postfilt_gain of 0.88770.  This postfilt_gain is applied   */
...
static void hpf60(postfilt_context* p, int16_t speech[])
{
	...
	iir_2nd_d(speech, hpf60_den, hpf60_num, speech, p->hpf60_delin,
		  p->hpf60_delout_hi, p->hpf60_delout_lo, FRAME);
}
...
static void lpf3500(postfilt_context* p, int16_t speech[])
{
	...
	iir_2nd_d(speech, lpf3500_den, lpf3500_num, speech, p->lpf3500_delin,
		  p->lpf3500_delout_hi, p->lpf3500_delout_lo, FRAME);
}


/* ======================================= */
/* postfilt_init(): this is to workaround the TI
 * compiler feature to omit static and global variables zeroing */
/* ======================================= */
void* postfilt_init(void* context)
{
	
	int16_t temp = 0; /* Q15 */
	register int16_t i;
	postfilt_context* p = (postfilt_context*) context;
	if (!p) p = (postfilt_context*) malloc(sizeof(postfilt_context));
	if (!p) return 0;
	memset(p, 0x00, sizeof(postfilt_context));
