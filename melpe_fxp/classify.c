...
typedef struct _classify_context {
	int16_t bpfdel[BPF_ORD + BPF_ORD / 3];
	int16_t back_sigbuf[PIT_COR_LEN - PIT_SUBFRAME];
	BOOLEAN firstTime;
} classify_context;
...
**
*****************************************************************************/
void classify(void* melpe_handle, int16_t inbuf[], classParam * classStat, int16_t autocorr[])
{
	register int16_t i, j;
	melpe_context* m = (melpe_context*) melpe_handle;
	classify_context* c = m->classify_handle;
	...
	if (c->firstTime) {
		g->voicedEn = FIVE_Q11;/* Both voicedEn and silenceEn are 1/10 of */
		g->silenceEn = THREE_Q11;/* their values in floating point version. */
		...
		v_equ(&(sigbuf_out[BPF_ORD / 3 - PIT_COR_LEN + PIT_SUBFRAME]),
		      c->back_sigbuf, PIT_COR_LEN - PIT_SUBFRAME);
	}
	...
	for (i = 0; i < BPF_ORD / 2; i++) {
		/*      iir_2nd_s(&(sigbuf[BPF_ORD/3]), &bpf_den[i*3], &bpf_num[i*3],
		   ...
		   sigbuf_len); */
		v_equ(sigbuf_in, &(c->bpfdel[2 * i]), BPF_ORD / 3);
		v_equ(sigbuf_out, &(c->bpfdel[2 * i + 2]), BPF_ORD / 3);
		...
		v_equ(&(c->bpfdel[2 * i]), &(sigbuf_in[sigbuf_len]), BPF_ORD / 3);
	...
	v_equ(&(c->bpfdel[BPF_ORD]), &(sigbuf_out[PIT_COR_LEN]), BPF_ORD / 3);

	v_equ(c->back_sigbuf, &(sigbuf_out[BPF_ORD / 3 + PIT_SUBFRAME]),
	...
	if (g->silenceEn > melpe_sub(g->voicedEn, X15_Q11))
		g->silenceEn = melpe_sub(g->voicedEn, X15_Q11);
	...
	if (classStat->subEnergy < SILENCE_DB_Q11)
		classy = SILENCE;
		 ...
		 interp_scalar(g->voicedEn, g->silenceEn, X065_Q15)) {
		...
		if ((lowBandCorx > X05_Q15) ||
			...
			classy = VOICED;
			 ...
			 interp_scalar(g->voicedEn, g->silenceEn, X03_Q15)) {
		...
		if ((lowBandCorx > X055_Q15) ||
			...
			classy = VOICED;
			  ...
			  interp_scalar(g->voicedEn, g->silenceEn, X06_Q15)) &&
...
static void frac_cor(int16_t inbuf[], int16_t pitch, int16_t * cor)
{
	...
	long40_t ACC_r0, ACC_rk, ACC_A;	/* Emulating 40Bit-Accumulator */
...
/* ================================================= 
 * classify_init(): Constructor of classify object
 * context is already created context or NULL if not created
 * return value is context pointer
 * To release context call free() on this pointer 
 * ================================================= */
void* classify_init(void* classify_handle)
{
	classify_context* c = (classify_context*) classify_handle;
	if (!c) c = (classify_context*) malloc(sizeof(classify_context));
	if (!c) return 0;
	memset(c, 0x00, sizeof(classify_context));
	c->firstTime = TRUE;
	return (void*) c;
}
