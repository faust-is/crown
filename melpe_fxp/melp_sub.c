...
typedef struct _melp_sub_context { 
	int16_t bpfsp[NUM_BANDS][PITCH_FR - FRAME]; /* bpvc_ana */
	int16_t bpfdelin[NUM_BANDS][BPF_ORD];
	int16_t bpfdelout[NUM_BANDS][BPF_ORD];
	int16_t envdel[NUM_BANDS][ENV_ORD];
	int16_t envdel2[NUM_BANDS];
} melp_sub_context;
...
void bpvc_ana(void* melp_sub_handle, int16_t speech[], int16_t fpitch[], int16_t bpvc[],
	      int16_t * pitch)
{
	melp_sub_context* s = (melp_sub_context*) melp_sub_handle;
	...
	v_equ(&sigbuf[BPF_ORD], s->bpfsp[0], PITCH_FR - FRAME);
	...
	for (section = 0; section < BPF_ORD / 2; section++) {
		iir_2nd_s(&sigbuf[BPF_ORD + PITCH_FR - FRAME],
			  ...
			  &s->bpfdelin[0][section * 2], &s->bpfdelout[0][section * 2],
			  FRAME);
	}
	v_equ(s->bpfsp[0], &sigbuf[BPF_ORD + FRAME], PITCH_FR - FRAME);
	...
	*pitch = frac_pch(&sigbuf[BPF_ORD + PITCHMAX], bpvc, fpitch[0], 5,
	...
	for (i = 1; i < NUM_BANDS; i++) {
		...
		v_equ(&sigbuf[BPF_ORD], s->bpfsp[i], PITCH_FR - FRAME);
		v_equ(&sigbuf[BPF_ORD + PITCH_FR - FRAME],
		      &speech[PITCH_FR - FRAME - PITCHMAX], FRAME);
		...
		for (section = 0; section < BPF_ORD / 2; section++) {
			...
			iir_2nd_s(&sigbuf[BPF_ORD + PITCH_FR - FRAME],
				  ...
				  &sigbuf[BPF_ORD + PITCH_FR - FRAME],
				  &s->bpfdelin[i][section * 2],
				  &s->bpfdelout[i][section * 2], FRAME);
		}
		v_equ(s->bpfsp[i], &sigbuf[BPF_ORD + FRAME], PITCH_FR - FRAME);
		...
		temp = melpe_shr(s->envdel2[i], scale);
		s->envdel2[i] =
		    melpe_shr(sigbuf[BPF_ORD + FRAME - 1], (int16_t) (-scale));
		v_equ_shr(&sigbuf[BPF_ORD - ENV_ORD], s->envdel[i], scale,
			  ENV_ORD);
		envelope(&sigbuf[BPF_ORD], temp, &sigbuf[BPF_ORD], PITCH_FR);
		v_equ_shr(s->envdel[i], &sigbuf[BPF_ORD + FRAME - ENV_ORD],
...
void q_gain(int16_t * gain, int16_t * gain_index, int16_t gn_qlo,
	    ...
	    int16_t double_flag, int16_t scale, int16_t* prev_gain)
{
	int16_t temp, temp2;
	int16_t q_gain_prev_gain = *prev_gain;
	...
	temp = melpe_add(melpe_shr(gain[1], 1), melpe_shr(q_gain_prev_gain, 1));
	if (melpe_abs_s(melpe_sub(gain[1], q_gain_prev_gain)) < GAIN_INT_DB_Q8 &&
...
void q_gain_dec(int16_t * gain, int16_t * gain_index, int16_t gn_qlo,
		...
		int16_t * prev_gain, int16_t * prev_gain_err)
{
	int16_t temp, temp2;
	int16_t q_gain_dec_prev_gain = *prev_gain;
	...
	if (gain_index[0] == 0) {
		...
		temp = melpe_sub(gain[1], q_gain_dec_prev_gain);
		...
		gain[0] = melpe_add(melpe_shr(gain[1], 1), melpe_shr(q_gain_dec_prev_gain, 1));
	} else {
...
void scale_adj(int16_t * speech, int16_t gain, int16_t length,
	       int16_t scale_over, int16_t inv_scale_over, int16_t* prev_scale_ptr)
{
	register int16_t i;
	...
	for (i = 1; i < scale_over; i++) {
		...
		interp1 = melpe_L_mult(*prev_scale_ptr, temp);	/* interp1 in Q29 */
	...
	*prev_scale_ptr = scale;

	v_free(tempbuf);
}

/* ======================================= */
/* melp_sub_init(): this is to workaround the TI
 * compiler feature to omit static and global variables zeroing */
/* ======================================= */
void* melp_sub_init(void* context)
{
	melp_sub_context* s = (melp_sub_context*) context;
	if (!s) s = (melp_sub_context*) malloc(sizeof(melp_sub_context));
	if (!s) return 0;
	memset(s, 0x00, sizeof(melp_sub_context));
	return (void*) s;
}
