...
#define pp (Word16) (LPCORD)
#define nc (Word16) (LPCORD >> 1)
...
void Autocorr(Word16 x[], Word16 p, Word16 r_h[], Word16 r_l[])
{
  ...
  for(i=0; i<L_window; i++)
    y[i] = mult_r(x[i], hamming_window[i]);
...
#else
      sum = L_mac0_overflow_test(sum, y[i], y[i], &OFlag);
#endif
    }
...
void Fac_Pond(Word16 gamma, Word16 fac[])
{
  ...
  fac[0] = gamma;
  for(i=1; i<pp; i++)
    fac[i] = r_ound( L_mult(fac[i-1], gamma) );
...
void Pond_Ai(Word16 a[], Word16 fac[], Word16 a_exp[])
{
  ...
  a_exp[0] = a[0];
  for(i=1; i<=pp; i++)
    a_exp[i] = r_ound( L_mult(a[i], fac[i-1]) );
