...
{
  Word16 i;
  static const Word16 bitno[PARAMETERS_SIZE] = {8, 9, 9,            /* split VQ LSP  */
          ...
          Word16 code[], Word16 L_subfr, Word16 *gain_pit, Word16 *gain_cod)
{
   ...
   *-----------------------------------------------*/
   ...
   if(bfi != 0)
   {
     dc->last_ener_pit = sub(dc->last_ener_pit, (Word16)128);	/* -0.5 in Q8 */
     if(dc->last_ener_pit < 0) dc->last_ener_pit = 0;

     dc->last_ener_cod = sub(dc->last_ener_cod, (Word16)128);	/* -0.5 in Q8 */
     if(dc->last_ener_cod < 0) dc->last_ener_cod = 0;
   }
   else
   {
     ...
     L_tmp = Load_sh(dc->last_ener_pit, (Word16)8);	/* .5 last_ener_pit in Q9  */
     L_tmp = add_sh(L_tmp,dc->last_ener_cod, (Word16)7); /*+.25 last_ener_code 
											in Q9    */
     ...
     L_tmp = Load_sh(dc->last_ener_cod, (Word16)8);	/* .5 last_ener_cod in Q9  */
     L_tmp = add_sh(L_tmp, dc->last_ener_pit, (Word16)7); /*+.25 last_ener_pit 
											in Q9    */
     L_tmp = sub_sh(L_tmp, (Word16)768, (Word16)9); /* -3.0 in Q9          */
     if(L_tmp < 0) L_tmp = 0;
     ...
     dc->last_ener_pit = add(t_qua_ener[j],   pred_pit);
     dc->last_ener_cod = add(t_qua_ener[j+1], pred_cod);

     /* Limit energies ->for transmission errors */

     if(sub(dc->last_ener_pit, (Word16)6912)>0) dc->last_ener_pit = 6912; 
									/* 6912 = 27 in Q8 */
     if(sub(dc->last_ener_cod, (Word16)6400)>0) dc->last_ener_cod = 6400; 
									/* 6400 = 25 in Q8 */
  }
  ...
  L_tmp = Load_sh(dc->last_ener_pit, (Word16)6);	 /* last_ener_pit/2 in Q15 */
  ...
  *gain_pit = extract_l(L_tmp);
  ...
  L_tmp = Load_sh(dc->last_ener_cod, (Word16)6);	/* last_ener_cod/2 in Q15 */
                ...
                Word16 *gain_pit, Word16 *gain_cod)
{
  ...
  L_tmp = Load_sh(ctx->last_ener_pit, (Word16)8); /* .5 last_ener_pit in Q9     */
  L_tmp = add_sh(L_tmp, ctx->last_ener_cod, (Word16)7); /*+.25 last_ener_code 
											in Q9    */
  L_tmp = sub_sh(L_tmp, (Word16)768, (Word16)9); /* -3.0 in Q9             */
  if(L_tmp < 0) L_tmp = 0;
  pred_pit = store_hi(L_tmp, (Word16)7);	/* result in Q8 			   */
  ...
  L_tmp = Load_sh(ctx->last_ener_cod, (Word16)8); /* .5 last_ener_cod  in Q9    */
  L_tmp = add_sh(L_tmp, ctx->last_ener_pit, (Word16)7); /*+.25 last_ener_pit
											in Q9    */
  L_tmp = sub_sh(L_tmp, (Word16)768, (Word16)9); /* -3.0 in Q9             */
  if(L_tmp < 0) L_tmp = 0;
  pred_cod = store_hi(L_tmp, (Word16)7);	/* result in Q8 			   */
  ...
  j = shl(index, (Word16)1);
  ctx->last_ener_pit = add(t_qua_ener[j],   pred_pit);
  ctx->last_ener_cod = add(t_qua_ener[j+1], pred_cod);

  /* Limit energies ->for transmission errors */

  if(sub(ctx->last_ener_pit, (Word16)6912) > 0) ctx->last_ener_pit = 6912;
									/* 6912 = 27 in Q8 */
  if(sub(ctx->last_ener_cod, (Word16)6400) > 0) ctx->last_ener_cod = 6400;
									/* 6400 = 25 in Q8 */
  ...
  L_tmp = Load_sh(ctx->last_ener_pit, (Word16)6);	/* last_ener_pit/2 in Q15 */
  L_tmp = sub_sh(L_tmp, ener_plt, (Word16)6);	/* - ener_plt/2    in Q15 */
  L_tmp = add_sh(L_tmp, (Word16)12, (Word16)15); /* to have gain in Q12   */
  L_extract(L_tmp, &exp, &frac);
  L_tmp = pow2(exp, frac);
  ...
  *gain_pit = extract_l(L_tmp);
  ...
  L_tmp = Load_sh(ctx->last_ener_cod, (Word16)6);	/* last_ener_cod/2 in Q15 */
  L_tmp = sub_sh(L_tmp, ener_c, (Word16)6);	/* - ener_c/2      in Q15 */
  L_extract(L_tmp, &exp, &frac);
  L_tmp = pow2(exp, frac);
  *gain_cod = extract_l(L_tmp);
...
Word16 G_Code(Word16 xn2[], Word16 y2[], Word16 L_subfr)
{
   ...
   OFlag = 0;
#ifdef HW_OVERFLOW_BIT_SUPPORTED
   (void) hw_reset_overflow();
#endif
...
#else
     s = L_mac0_overflow_test(s, xn2[i], y2[i], &OFlag);
#endif
   }
...
#else
     s = L_mac0_overflow_test(s, y2[i], y2[i], &OFlag);
#endif
...
#else
     s = L_mac0_overflow_test(s, xn[i], y1[i], &OFlag);
#endif
...
#else
     s = L_mac0_overflow_test(s, y1[i], y1[i], &OFlag);
#endif
...
#define LOCAL_PIT_MAX 142 /* warning: pit_max=143 in scod_tet.c and sdec_tet.c */
...
Word16 Pitch_Ol_Dec(Word16 signal[], Word16 L_frame)
{
...
#endif
   t0 = 0;
   for(i= -LOCAL_PIT_MAX; i< L_frame; i+=2)
#ifdef HW_OVERFLOW_BIT_SUPPORTED
     t0 = L_mac0(t0, signal[i], signal[i]);
#else
     t0 = L_mac0_overflow_test(t0, signal[i], signal[i], &OFlag);
#endif
   ...
   *                                                                    *
   *  First section:  lag delay = LOCAL_PIT_MAX to 80                         *
   *  Second section: lag delay = 79 to 40                              *
   *  Third section:  lag delay = 39 to 20                              *
   *--------------------------------------------------------------------*/

   p_max1 = Lag_Max(signal, sig_dec, L_frame, (Word16)LOCAL_PIT_MAX,
									(Word16)80 , &max1);
...
void Pre_Process(Pre_Process_Context* pre_process, Word16 signal[], Word16 lg)
{
  ...
  {
     x1 = pre_process->x0;
     pre_process->x0 = signal[i];

     L_tmp     = Load_sh(pre_process->x0, (Word16)15);
     L_tmp     = sub_sh(L_tmp, x1, (Word16)15);
     L_tmp     = L_mac(L_tmp, pre_process->y_hi, (Word16)32735);
     L_tmp     = add_sh(L_tmp, mult(pre_process->y_lo, (Word16)32735), (Word16)1);
     signal[i] = extract_h(L_tmp);
     pre_process->y_hi      = extract_h(L_tmp);
     pre_process->y_lo      = extract_l(sub_sh(L_shr( L_tmp,(Word16)1 ), pre_process->y_hi, (Word16)15));
  }
  return;
}
